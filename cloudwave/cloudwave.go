...
type Config struct {
	...
	DontSupportAddColumn   bool //翰云
	DontSupportAlterColumn bool //翰云
}
...
func Open(dsn string) gorm.Dialector {
	dsnConf, _ := cloudwave.ParseDSN(dsn)
	return &Dialector{Config: &Config{DSN: dsn, DSNConfig: dsnConf}}
}

func OpenDB(dsn string) (*gorm.DB, error) {
	db, err := gorm.Open(New(Config{
		DriverName:        "cloudwave",
		DSN:               dsn,
		DefaultStringSize: 256,
	}), &gorm.Config{
		//PrepareStmt:true,//global mode,all operations will create prepared stmt and cache to speed up
...
func New(config Config) gorm.Dialector {
	...
	case config.DSN != "" && config.DSNConfig == nil:
		config.DSNConfig, _ = cloudwave.ParseDSN(config.DSN)
	}
...
func (dialector Dialector) Initialize(db *gorm.DB) (err error) {
	if dialector.DriverName == "" {
		dialector.DriverName = "cloudwave"
	}
	...
	if !dialector.Config.SkipInitializeWithVersion {
		...
		//翰云数据库设置如下：
		//dialector.Config.DisableDatetimePrecision = true      //禁用 datetime 精度
		dialector.Config.DontSupportRenameIndex = true  //不支持重命名索引
		dialector.Config.DontSupportRenameColumn = true //不支持重命名列
		//dialector.Config.DontSupportForShareClause = true     //不支持
		dialector.Config.DontSupportNullAsDefaultValue = true //不支持將字符串改为NULL
		//dialector.Config.DontSupportRenameColumnUnique = true //不支持字段UNIQUE

		dialector.Config.DontSupportAddColumn = true
		dialector.Config.DontSupportAlterColumn = true
	}
...
func (dialector Dialector) getSchemaTimeType(field *schema.Field) string {
	...
	//return "datetime" + precision + " NULL" //weip
	return "datetime" + precision
}
